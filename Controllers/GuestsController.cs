...
public class GuestsController : Controller
{
	...
	public async Task<ObjectResult> Create([FromBody] Guest guest)
	{
		...
		return validationResult.IsValid 
			? Ok((await Crud.Add(true, guest))[0]) 
	...
	public async Task<ObjectResult> Update([FromBody] Guest guest)
	{
		...
		var validationResult = Validator.Validate(guest);
		return validationResult.IsValid
			? Ok(await Crud.Delete(new HashSet<Key>([new(nameof(Guest.Id), guest.Id)])).ContinueWith(async _ => (await Crud.Add(true, guest))[0]))
			: BadRequest(validationResult.Errors);
	}
	...
	public async Task<ObjectResult> Delete([FromRoute] int Id)
	{
		var accessToken = HttpContext.AccessToken();
		if (accessToken is null) return BadRequest(HttpContextExtensions.MissingAccessTokenException);
		var guest = await Crud.Get(new HashSet<Key>([new(nameof(Guest.Id), Id)]));
		var requirement = new RoleRequirement(RoleRequirement, await GetScheduleId(guest), UserRoles.Admin, UserRoles.Owner, UserRoles.Manager);
		var result = await UserProcessor.Process(accessToken, requirement);
		return result.IsAuthorized 
			? Ok(await Crud.Delete(new HashSet<Key>([new(nameof(Guest.Id), Id)]))) 
